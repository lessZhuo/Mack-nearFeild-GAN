...
class ImageDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        mask = np.array(mask.mean(0))
        label = np.array(label.mean(0))
        image_A = Image.fromarray(np.uint8(mask * 255)).convert('RGB')
        image_B = Image.fromarray(np.uint8(label * 255)).convert('RGB')


        # if self.unaligned:      # 为了AB对齐，关闭了不对齐，所以只改了else
        #     # image_B = Image.open(self.files_B[random.randint(0, len(self.files_B) - 1)])
        #     matrix_B = pd.read_csv(lzma.open(self.files_B[random.randint(0, len(self.files_B) - 1)], mode='rb'),
        #                            header=None)
        #     result_B = np.array(matrix_B)  # 生成np 的array
        #     image_B = Image.fromarray(np.uint8(result_B*255))  # 从数据，生成image对象
        # else:
        #     # image_B = Image.open(self.files_B[index % len(self.files_B)])
        #     # matrix_B = pd.read_csv(lzma.open(self.files_B[index % len(self.files_B)], mode='rb'), header=None)
        #     # result_B = np.array(matrix_B)  # 生成np 的array
        #     # image_B = Image.fromarray(np.uint8(result_B*255))  # 从数据，生成image对象
        #
        #     image_B = Image.fromarray(np.uint8(xx_r*255)).convert('RGB')    # convert('RGB'), xx_r, xx_i, yy_r, yy_i
    ...
    def center_crop(self, numpy_data, f, l):
        ...
        for i in range(v):
            if i == 0:
                ...
                num = np.append(num, numpy_data, axis=1)
            if i == (v - 1):
                for j in range(h):
                    if j == 0:
                        num_v = num
                    else:
                        num_v = np.append(num_v, num, axis=2)
        num_r = num_v.shape[1]
        num_v1 = num_v.shape[2]
